...
export class UploadComponent{
  ...
  onUpload(): void{
    ...
    const url = `api/files/upload?m=${this.isLimited ? `2&p=${this.pin}`: '1'}&st=${this.SaveTime*60*60}&dc=${this.MaxCountDownload}`;
    if(this.isLimited) {
  ...
  private chekLimit(): boolean{
    const url = `api/files/check/limit?fs=${this.file().size}`;
    let res = $.ajax({
      url: url,
      async: false,
      method: 'GET'
  ...
  onDelete(obj: FileModel): void{
    const url = `api/files/delete/${obj.downloadKey}/${obj.name}`;
    $.ajax({
      url: url,
      method: 'POST',
      success: data => {
  ...
  openGroupMenu(): void{
    this.modalRef = this.modalService.open(ModalWorkingGroup, { data: {
        isModal: true
      }});
  }
}
